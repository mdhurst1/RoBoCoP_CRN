...
int main()
{
	...
	WeartheringRates.push_back(0.005);
	WeartheringRates.push_back(0.5);
	...
	for (int a=0; a<TidalRanges.size(); ++a)
	{
		for (int b=0; b<WaveHeights.size(); ++b)
		{
			for (int c=0; c<WeatheringRates.size(); ++c)
			{
				//setup the output file
				PlatformModel.OutputFileName = "ShoreProfile_T"+tostr(TidalRanges[a])+"_H"+tostr(WaveHeights[b])+"_W"+tostr(WeatheringRates[c])+".xz";
		
				//reset the model
				PlatformModel.Initialise();
